...
const invitationSelect = {
  ...
  appointment: {
    select: { referenceId: true, scheduledDate: true, scheduledTime: true },
  },
...
const expireIfNeeded = async <
  T extends { id: string; status: string; expiresAt: Date },
>(
  invitation: T,
) => {
  if (
...
const shareDetails = (
  token: string,
  patient: { parentName: string; patientName: string },
  phone: string,
  expiresAt: Date,
) => {
  ...
  return {
    ...
    whatsappUrl: `https://wa.me/${normalizeWhatsAppPhone(phone)}?text=${encodeURIComponent(message)}`,
  };
};

export const listEligiblePatients = (search?: string) =>
  prisma.patient.findMany({
    where: {
      appointments: { some: buildFeedbackEligibleAppointmentWhere() },
      ...(search
        ? {
            OR: [
...
export const verifyAppointmentFeedback = async (
  patientId: string,
  ...
  return {
    ...
    submittedAt:
      invitation?.response?.submittedAt.toISOString() ??
      invitation?.submittedAt?.toISOString() ??
      null,
  };
};
...
export const createFeedbackInvitation = async (
  adminId: string,
  input: { patientId: string; appointmentReferenceId?: string },
) => {
  ...
  const submitted = await prisma.feedbackInvitation.findFirst({
    ...
    orderBy: { createdAt: "desc" },
    select: { referenceId: true, response: { select: { id: true } } },
  });
  if (submitted?.response)
    return {
      outcome: "FEEDBACK_ALREADY_SUBMITTED" as const,
      invitationReferenceId: submitted.referenceId,
      responseId: submitted.response.id,
    };
  const existing = await prisma.feedbackInvitation.findFirst({
    where: {
      patientDbId: patient.id,
      appointmentRequestId: appointment.id,
  ...
  const invitation = await prisma.feedbackInvitation.create({
    data: {
      ...
      patientDbId: patient.id,
      appointmentRequestId: appointment.id,
      recipientPhone: patient.primaryPhone,
      expiresAt,
  ...
  return {
    ...
    invitation,
    share: shareDetails(token, patient, patient.primaryPhone, expiresAt),
  };
};

export const listFeedbackInvitations = async (query: InvitationListQuery) => {
  await prisma.feedbackInvitation.updateMany({
    where: {
  ...
  const [items, total] = await prisma.$transaction([
    ...
    prisma.feedbackInvitation.count({ where }),
  ]);
  return {
    items,
    pagination: {
...
export const getFeedbackInvitation = async (referenceId: string) => {
  const invitation = await prisma.feedbackInvitation.findUnique({
    where: { referenceId },
    select: invitationSelect,
  });
...
export const listFeedbackResponses = async (query: ResponseListQuery) => {
  ...
  const select = {
    ...
    moderatedAt: true,
    patient: {
      select: { patientId: true, patientName: true, parentName: true },
    },
    invitation: { select: { referenceId: true } },
  ...
  const [items, total] = await prisma.$transaction([
    ...
    prisma.parentFeedbackResponse.count({ where }),
  ]);
  return {
    items,
    pagination: {
...
export const getFeedbackResponse = (id: string) =>
  prisma.parentFeedbackResponse.findUnique({
    where: { id },
    include: {
      patient: {
        select: { patientId: true, patientName: true, parentName: true },
...
export const moderateFeedbackResponse = async (
  id: string,
  status: "APPROVED" | "REJECTED",
  adminId: string,
) => {
  const response = await prisma.parentFeedbackResponse.findUnique({
    where: { id },
  });
  if (!response) return { outcome: "NOT_FOUND" as const };
  if (status === "APPROVED" && !response.consentToPublish)
    return { outcome: "CONSENT_REQUIRED" as const };
  const updated = await prisma.parentFeedbackResponse.update({
    where: { id },
    data: {
