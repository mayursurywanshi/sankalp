...
export const getAppointmentSummary = async () =>
  parse<AppointmentSummary>(
    await authorizedFetch("/api/admin/appointments/summary"),
  );
export const getAppointmentDetail = async (id: string) =>
  parse<AppointmentDetail>(
    await authorizedFetch(`/api/admin/appointments/${encodeURIComponent(id)}`),
  );
export const getAssignableDoctors = async (): Promise<Doctor[]> =>
  (await fetchDoctors()).filter(
    (doctor) => doctor.isActive && doctor.credentialStatus === "ACTIVE",
  );
export const getDoctorAvailability = async (
  ...
  if (excludeReferenceId) query.set("excludeReferenceId", excludeReferenceId);
  return parse<SlotAvailability>(
    await authorizedFetch(`/api/admin/appointments/availability?${query}`),
  );
};
export const assignAppointment = async (
  ...
  parse<Appointment>(
    await authorizedFetch(
      `/api/admin/appointments/${encodeURIComponent(id)}/assignment`,
      {
        method: "POST",
