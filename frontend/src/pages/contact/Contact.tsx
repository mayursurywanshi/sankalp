...
export const Contact = () => {
  ...
  useEffect(() => {
    ...
    getContactContent()
      ...
      .catch(() => {
        if (isMounted)
          setLoadError("We could not load the contact page. Please try again.");
      });
    return () => {
      isMounted = false;
    };
  ...
  if (loadError)
    return (
      <main className="contact-status" role="alert">
        <h1>Unable to load Contact Us</h1>
        <p>{loadError}</p>
      </main>
    );
  ...
  return (
    <main className="contact-page">
      <section className="contact-hero" aria-labelledby="contact-title">
        ...
        <p>
          {content.hero.description} Reach out and our team will guide you
          toward the right support for your child.
        </p>
      </section>
      ...
      <section className="contact-layout" aria-label="Contact Sankalp">
        <article className="contact-panel contact-combined">
          <section
            className="contact-details contact-inner-card"
            aria-labelledby="contact-details-title"
          >
            <header>
              <span>Connect with us</span>
              <h2 id="contact-details-title">We’re here for your family</h2>
            </header>
            <div className="contact-details__list">
              {content.details.map((detail) => (
                <div className="contact-detail" key={detail.id}>
                  <span
                    className={`contact-detail__icon contact-detail__icon--${detail.id}`}
                    aria-hidden="true"
                  >
                    <DetailIcon type={detail.id} />
                  </span>
                  <div>
          ...
          <section
            className="contact-form-card contact-inner-card"
            ...
            <form onSubmit={submitForm} noValidate>
              ...
              <div className="contact-message-field">
                ...
                <span
                  ...
                  className="contact-message-count"
                  aria-live="polite"
                >
        ...
        <article className="contact-panel contact-map">
          <iframe
            ...
            title={`Location of ${content.map.label}`}
            allowFullScreen
            loading="lazy"
