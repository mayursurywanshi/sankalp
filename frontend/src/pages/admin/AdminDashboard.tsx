      ...
      {error ? <section className="admin-error" role="alert"><span>🌤️</span><h1>Dashboard needs a moment</h1><p>{error}</p><button type="button" onClick={() => void loadDashboard()}>Try Again</button></section> : data && <>
        ...
        <div className="admin-dashboard-grid">
          ...
          <div className="admin-dashboard-stack">
            <section className="admin-panel admin-schedule"><header><div><small>TODAY</small><h2>Schedule Summary</h2></div><strong>{data.todaySchedule.total}</strong></header><div className="admin-schedule__body"><div className="admin-donut" style={{ "--requested": data.todaySchedule.total ? `${data.todaySchedule.requested / data.todaySchedule.total * 100}%` : "0%" } as React.CSSProperties}><span>{data.todaySchedule.total}<small>Visits</small></span></div><ul><li><i className="is-confirmed" />Confirmed <b>{data.todaySchedule.confirmed}</b></li><li><i className="is-requested" />Requested <b>{data.todaySchedule.requested}</b></li><li><i className="is-completed" />Completed <b>{data.todaySchedule.completed}</b></li><li><i className="is-cancelled" />Cancelled <b>{data.todaySchedule.cancelled}</b></li></ul></div></section>
