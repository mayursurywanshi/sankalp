...
} from "./admin-appointments.validation";
...
export const getSummary = async (
  ...
  try {
    response
      .status(200)
      .json({ success: true, data: await getAppointmentSummary() });
  } catch (error) {
    console.error("Unable to summarize appointments", error);
    response
      .status(500)
      .json({ success: false, message: "Unable to load appointment summary." });
  }
};
...
export const getAvailability = async (
  ...
  try {
    const result = await getDoctorAvailability(
      ...
      validation.data.excludeReferenceId,
    );
    if (result.outcome === "DOCTOR_NOT_FOUND") {
      response
        .status(404)
        .json({ success: false, message: "Active Doctor was not found." });
      return;
    }
    ...
    response
      .status(500)
      .json({ success: false, message: "Unable to load Doctor availability." });
  }
};
...
export const getAppointment = async (
  ...
  try {
    ...
    response
      .status(500)
      .json({ success: false, message: "Unable to load appointment details." });
  }
};
...
export const postAssignment = async (
  ...
  try {
    ...
    const errors = {
      NOT_FOUND: [404, "Appointment request was not found."],
      CLOSED: [409, "Completed or cancelled appointments cannot be assigned."],
      DOCTOR_UNAVAILABLE: [
        409,
        "Select an active Doctor with active login credentials.",
      ],
      INVALID_DATE: [400, "Select a future clinic working date."],
      SLOT_CONFLICT: [
        409,
    ...
    response
      .status(500)
      .json({ success: false, message: "Unable to assign appointment." });
  }
};
...
export const patchStatus = async (
  ...
  try {
    ...
    const errors = {
      NOT_FOUND: [404, "Appointment request was not found."],
      CLOSED: [409, "This appointment is already closed."],
      CASE_HISTORY_REQUIRED: [
        409,
        "The assigned Doctor must create case history before completion.",
      ],
    } as const;
