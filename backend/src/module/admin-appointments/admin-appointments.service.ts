...
export const getAppointmentSummary = async () => {
  const grouped = await prisma.appointmentRequest.groupBy({
    by: ["status"],
    _count: { _all: true },
  });
...
export const getDoctorAvailability = async (
  ...
  return clinicSlotAvailability(doctor.id, date, excludeReferenceId);
};
...
export const getAdminAppointment = async (referenceId: string) => {
  ...
  return {
    ...
    logs: appointment.logs.map((log) => ({
      ...log,
      createdAt: log.createdAt.toISOString(),
    })),
    caseHistory: appointment.caseHistory
      ? {
          ...appointment.caseHistory,
          appointmentDate: formatDate(appointment.caseHistory.appointmentDate),
          nextAppointmentDate: formatDate(
            appointment.caseHistory.nextAppointmentDate,
          ),
          createdAt: appointment.caseHistory.createdAt.toISOString(),
          updatedAt: appointment.caseHistory.updatedAt.toISOString(),
        }
      : null,
...
export const assignAppointment = async (
  ...
  if (
    !doctor ||
    !doctor.isActive ||
    doctor.credentialStatus !== "ACTIVE" ||
    !doctor.login?.isActive
  )
  ...
  const event = appointment.assignedDoctorId
    ? "DOCTOR_REASSIGNED"
    : "DOCTOR_ASSIGNED";
  const updated = await prisma.$transaction(async (transaction) => {
    const value = await transaction.appointmentRequest.update({
      where: { id: appointment.id },
      data: {
        assignedDoctorId: doctor.id,
        scheduledDate,
        scheduledTime: input.scheduledTime,
        assignmentNote: input.note,
        assignedAt: new Date(),
    ...
    await transaction.appointmentLog.create({
      data: {
        ...
        newStatus: "ASSIGNED",
        previousDoctorId: appointment.assignedDoctorId,
        assignedDoctorId: doctor.id,
        event,
        note: input.note,
  ...
  return {
    ...
    appointment: presentAppointment(updated),
  };
};
...
export const updateAppointmentStatus = async (
  ...
  if (
    status === "COMPLETED" &&
    (!appointment.assignedDoctorId || !appointment.caseHistory)
  )
    return { outcome: "CASE_HISTORY_REQUIRED" as const };
  const event =
  ...
  const updated = await prisma.$transaction(async (transaction) => {
    ...
    if (status === "COMPLETED" && appointment.caseHistory)
      await transaction.patientCaseHistory.update({
        where: { id: appointment.caseHistory.id },
        data: { isLocked: true },
      });
    await transaction.appointmentLog.create({
      data: {
